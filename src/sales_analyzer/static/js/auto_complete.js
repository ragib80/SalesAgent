(function () {
  ...
  const AI_SUGGEST_MAX_LEN = 20; //  cap length to avoid calling when > 20 chars
  const AI_SUGGEST_MIN_LEN = 4;  //  lower bound before we ping AI
  ...
  $ghostPortal.css({ position: 'fixed', zIndex: 10020, display: 'none', pointerEvents: 'auto' });
  ...
  /* ---------- Generic AJAX helper for autocomplete (commit-aware) ---------- */
    ...
    $.getJSON(AUTOCOMPLETE_URL, { field, q, page })
      .done((resp) => {
        const items = (resp.results || []).map((r) => ({
          ...
          text: r.text,
          meta: r.meta || null,
          commit: r.commit || r.insert || r.value || null // optional commit value from server
        }));
  ...
  // ---- Field-term cleaner: supports multi-word values and stops at trailing context ----
  // Works for ANY label (Dealer, Brand, Product Name, etc.).
  function cleanFieldTerm(label, raw) {
    let t = (raw || '').trim();
    if (!t) return '';

    // Quoted values: e.g., Dealer "Delwar Paint" in last 6 months -> Delwar Paint
    if (t.startsWith('"')) {
      const lastQuote = t.lastIndexOf('"');
      if (lastQuote > 0) {
        const inner = t.slice(1, lastQuote);
    ...
    // Cut on obvious sentence enders
    t = t.replace(/[.;!?\n]+.*$/, '').trim();

    // Remove trailing context/prepositions
    ...
    // Remove trailing relative time phrases
    ...
    $.getJSON(AUTOCOMPLETE_URL, { field: normalizeFieldName(field), q, page })
      .done((resp) => {
        const items = (resp.results || []).map((r) => ({
          ...
          text: r.text,
          meta: r.meta || null,
          commit: r.commit || r.insert || r.value || null
        }));
        cb({ items, more: !!(resp.pagination && resp.pagination.more) });
      })
      .fail(() => cb({ items: [], more: false }));
  ...
  function insertLabeled(label, value) {
    ...
    const MULTI_VALUE_LABELS = new Set(['Division', 'Division Code', 'Material Group', 'Territory', 'Sales Zone']);
  ...
  // Commit-aware replacement: prefer item.commit; field-specific trimming rules (Dealer)
  function specialReplace(label, item) {
    ...
    const pickRaw = (x) => {
      ...
      if (typeof x === 'object') return x.commit || x.text || x.value || x.id || '';
      return '';
    };
    ...
    // --- Field-specific collapsing rules ---
    if (/^Dealer$/i.test(L)) {
      // Keep only "Name (Code)"; strip anything after a dash/pipe etc.
      commit = raw.replace(/\s*[-–—|]\s.*$/, '').replace(/,\s*$/, '').trim();
    }

    const result = insertLabeled(L, String(commit).trim());
    return result;
  }

  /* ---------- Wiring: keep dropdown above ghost & clickable ---------- */
  $ta.on('textComplete:show', function () {
    hideGhost();
    $ghostPortal.css('pointer-events', 'none');
  });
  $ta.on('textComplete:hide', function () {
    $ghostPortal.css('pointer-events', 'auto');
  });

  /* ---------- 0) Explicit "Label term" strategy (e.g., "Dealer delwar paint") ---------- */
  (function labeledFieldStrategy() {
    // ", Dealer delwar paint" | "Dealer: delwar paint" | "Dealer delwar paint"
  ...
  /* ---------- Generic catch-all strategy (context-aware search) ---------- */
  $ta.textcomplete(
    [
      {
        match: /([^\s,].*)$/,
        ...
        cache: false,
        context: function () {
          return false; // keep disabled by default unless we detect a label context below
        },
            ...
            if (curTerm.length >= 1) {
              currentRequest = searchField(curField, curTerm, curPage, ({ items, more }) => {
                ...
                requestAnimationFrame(() => {
                  const dd = document.querySelector('.textcomplete-dropdown');
                  if (!dd) return;
                  // make sure dropdown stays above ghost
        ...
        replace: function (item) {
          // Always route through specialReplace for consistent, bug-free insertion
          ...
          ${sugs
            .map(
              (s) => `<div class=\"ai-suggest-item py-1 px-2 rounded-2 mb-1 text-truncate\"
                        title=\"${s}\" style=\"cursor:pointer;\">${s}</div>`
            )
            .join('')}
        </div>
      </div>`;
  ...
  function placeGhostAboveInput() {
    ...
    $ghostPortal.find('.ai-suggest-card').css({ maxHeight: '200px', overflowY: 'auto' });
  ...
  function placeDropdownBelowInput() {
    ...
    // keep above ghost
    $dd.css({ top, left: r.left, width: r.width + 'px', visibility: 'visible', zIndex: 10050 });
  }
      ...
      [
        {
          ...
          search: function (t, callback) {
            ...
            if (ec && ec.label && ec.label !== 'Territory') {
              callback([]);
              return;
            }
      ...
      [
        {
          ...
          search: function (t, callback) {
            ...
            const ec = explicitContext();
            if (ec && ec.label && ec.label !== 'Sales Zone') {
              callback([]);
              return;
            }
          ...
          replace: function (item) {
            const result = specialReplace('Sales Zone', item); // explicit
            const $input = $('#message-input');
            $input.val(result).trigger('input');
            $input.trigger('textComplete:hide');
          },
      ...
      [
        {
          ...
          search: function (t, callback) {
            ...
            const ec = explicitContext();
            if (ec && ec.label && ec.label !== 'Material Group') {
              callback([]);
              return;
            }
  ...
  (function divisionStrategy() {
    ...
    const DIVISION_NAMES = [
      ...
      'Texbond',
      'Service',
      'Construction Chemica',
      'Food Grade Can',
      'Industrial Grade Can',
      'Container Scraps',
      'Coil Coating',
      'Training Service'
    ];
      ...
      [
        {
          ...
          replace: function (item) {
            const result = specialReplace('Division', item);
            const $input = $('#message-input');
            $input.val(result).trigger('input');
            $input.trigger('textComplete:hide');
          },
      ...
      [
        {
          ...
          search: function (t, callback) {
            ...
            const ec = explicitContext();
            if (ec && ec.label && ec.label !== 'Division Code') {
              callback([]);
              return;
            }
          ...
          replace: function (item) {
            const result = specialReplace('Division Code', item);
            const $input = $('#message-input');
            $input.val(result).trigger('input');
            $input.trigger('textComplete:hide');
          },
