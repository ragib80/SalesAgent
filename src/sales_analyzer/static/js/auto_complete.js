...
//new ghost 
  // === 🧠 AI Prompt Suggestion Logic ===
  const AI_SUGGEST_URL = '/api/sales/prompt-suggestions/';
  const $ghost = $('#ghost-suggestion');
  let aiTimer = null;

function showGhostSuggestions(suggestions) {
  if (!suggestions || !suggestions.length) {
    $ghost.stop(true, true).fadeOut(150);
    return;
  }
  ...
  const html = `
    <div class="card ai-suggest-card shadow-sm">
      <div class="card-body py-2 px-3">
        ...
        ${suggestions.map(s => `
          <div class="ai-suggest-item py-1 px-2 rounded-2 mb-1" style="cursor:pointer;">
            ${s}
          </div>
        `).join('')}
      </div>
  ...
  $ghost.html(html).stop(true, true).fadeIn(200);

  $ghost.find('.ai-suggest-item').on('click', function () {
    const suggestion = $(this).text().trim();
    $ta.val(suggestion).trigger('input');
    $ghost.stop(true, true).fadeOut(150);
  });
}
...
async function fetchAISuggestions(text) {
  try {
    const resp = await $.ajax({
      ...
      data: JSON.stringify({ 
        input_text: text,
        conversation_id: currentChatId  // 🆕 include chat id for history
      }),
      contentType: 'application/json',
    });
    showGhostSuggestions(resp.suggestions);
  } catch (err) {
  ...
  $ta.on('input', function() {
    ...
    $(SEND_BTN_SELECTOR).prop('disabled', !text);

    clearTimeout(aiTimer);
    if (text.length >= 10) {
      aiTimer = setTimeout(() => fetchAISuggestions(text), 600); // debounce 600ms
    } else {
      $ghost.fadeOut(150);
    }
  });
